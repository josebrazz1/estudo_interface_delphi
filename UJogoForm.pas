...
var
  ...
  i, memRam, memVideo, espacoDisco : Integer;
...
begin
  ...
  if cbDispositivo.ItemIndex = 0 then
    ...
    dispositivos := TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'PCs.ini')
  else
    {...caso contrário, seleciona o ini contendo os nomes de consoles}
    dispositivos := TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'Consoles.ini');

  {Instancia a StringList}
  iniSections := TStringList.Create;

  {Lê as seções do arquivo ini}
  dispositivos.ReadSections(iniSections);

  {Limpa a combo para novo preechimento}
  cbRequisitos.Clear;

  {Se o arquivo ini conter o trecho "PCs" então selleciona o respectivo ini}
  if dispositivos.FileName.Contains('PCs.ini') then
  begin
    for i := 0 to iniSections.Create.Count - 1 do
    begin
      memRam := dispositivos.ReadInteger(iniSections.Strings[i],'memRam',0);
      memVideo := dispositivos.ReadInteger(iniSections.Strings[i],'memVideo',0);
      espacoDisco := dispositivos.ReadInteger(iniSections.Strings[i],'espacoDisco',0);
